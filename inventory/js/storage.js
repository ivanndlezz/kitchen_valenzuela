...
function normalizeJsonProduct(p) {
  ...
  return {
    ...
    CostoEnvio: parseFloat(p["CostoEnvio"] || p.CostoEnvio || p.costoEnvio || p.costoTraslado || p.transferCost || p.transfer_cost) || 0,
    costoEnvio: parseFloat(p["CostoEnvio"] || p.CostoEnvio || p.costoEnvio || p.costoTraslado || p.transferCost || p.transfer_cost) || 0,
    ...
    quoteUtilityType: String(p["quote_utility_type"] || p.quoteUtilityType || "percent") === "amount" ? "amount" : "percent",
    quoteUtilityValue: parseFloat(p["quote_utility_value"] || p.quoteUtilityValue) || 0,
